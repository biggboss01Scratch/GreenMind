...
当前系统已完成传感器采集、触摸 UI、Wi-Fi/TCP 通信、网关握手、DeepSeek 调用、植物数据库、六种植物选择和分析记录等主要闭环。项目还建立了 18 张植物状态像素资源，并完成了 RGB565 分包、序号和 CRC32 校验协议。默认绿萝的三种状态图已内置到固件，仙人掌资源用于动态下载链路验证。当前界面仍以英文为主，完整中文 GB2312 字库虽已生成，但尚未链接到固件；水泵、土壤湿度传感器和中文长文本对话框也尚未接入。
...
7. 为后续增加中文、更多植物、土壤湿度和水泵安全控制保留清晰接口。
...
| 仙人掌动态图片 | 协议和软件测试通过 | 用于验证数据库查询、分包、CRC32 和 TFT 显示链路 |
| 其余植物动态图片 | 资源已生成，固件未全面自动下载 | 当前可能显示通用占位图，不应宣称全部已动态显示 |
| 中文 GB2312 字库 | 已准备但未接入 | 147204 字节，尚未进入 Keil 工程和 UI 渲染 |
| 完整 AI 文本对话框 | 尚未实现 | PC 日志有英文建议，STM32 目前只接收枚举结果 |
...
| `mint` | 薄荷 / MINT | *Mentha spicata* | 15～28 | 40～75 | 45～80 | EASY | BUILTIN |
...
| 仙人掌 | 作为动态图片协议的当前验证对象 |
| 薄荷、多肉、蝴蝶兰、番茄 | 资源和数据库记录已存在，固件当前未全面自动下载，可能使用通用占位图 |

因此汇报可以说“已经建立可扩展的 18 张资源库，并完成内置图和动态图两种路径”，但不能说“所有植物图片都已在实机自动下载显示”。
...
后端确定性规则是权威结果。即使模型返回了不一致的状态码，网关也不会让模型覆盖规则。完整英文解释限制为 160 个 ASCII 字符，并会处理换行、特殊引号、破折号和度数符号；格式错误时可重试或回退为规则生成的确定性建议。
...
PC 日志会显示较完整的 `suggestion_en`，但这段自然语言**尚未传给 TFT**。HOME 页 `MAIN ADVICE` 和 AI 页 `ADVICE` 目前展示的是有限建议码，如 `KEEP_CURRENT`、`MOVE_TO_SHADE`。
...
```text
Program Size: Code=42112 RO-data=33104 RW-data=236 ZI-data=14548
...
- 当前程序占用 Flash 约 `42112 + 33104 + 236 = 75452` 字节，约 73.7 KiB；
- 相对 512 KiB Flash，约占 14.4%；
- 当前 RAM 静态占用约 `236 + 14548 = 14784` 字节，约 14.4 KiB；
- 相对 64 KiB SRAM，约占 22.6%；
- 7 个警告来自原 TFT 驱动中的未使用局部变量，不是 GreenMind 新业务模块的编译错误；
- 中文字库尚未链接，因此以上体积不包含 147204 字节中文字库。
...
| T11 | 断网恢复 | 网关退出后显示 OFFLINE，重启后恢复 READY |
...
当前状态：**只完成字库生成和校验，尚未链接进 Keil 工程，也没有中文绘制函数。**

如果以后作为 `const` 只读数据链接，预计当前固件总 Flash 使用约为：

```text
75452 + 147204 = 222656 字节
```

约占 512 KiB Flash 的 42.5%，容量上可行；若按需逐字读取，RAM 增量可控制在 1 KiB 内。但这只是集成估算，不是当前构建结果。

### 17.2 个性化 AI 对话框

电脑端已经有植物相关的英文 `suggestion_en`，但 TFT 尚未收到这段文本。下一步建议：

1. 增加独立文本消息，而不破坏当前 `AI_RESULT`：

   ```text
   V1|AI_TEXT_BEGIN|REQ_ID|LENGTH|CRC
   V1|AI_TEXT_CHUNK|REQ_ID|SEQ|TEXT
   V1|AI_TEXT_END|REQ_ID|COUNT|CRC
   ```

2. 先支持 1～3 行短英文；
3. 再接入中文字库和 GB2312/UTF-8 转码；
4. 使用 Galgame 风格底部对话框逐行展示；
5. 文本失败时仍保留现有枚举结果，避免影响核心闭环。
...
当前系统仍属于课程项目原型：空气湿度不能替代土壤湿度，水泵尚未接入，完整 AI 文本和中文字库也尚未进入 TFT 运行路径，部分植物图片的动态加载仍需完成最终实机验收。后续将在保持“AI 只建议、不直接执行”的安全原则下，继续完善中文对话框、全植物资源、土壤湿度和本地水泵安全控制。
...
| `firmware/Assets/fonts/` | 已准备但未接入的中文字库 |
...
| “所有植物图片都从数据库下载到 STM32” | “18 张资源已建立；绿萝内置，仙人掌用于动态链路验证，其余待全面接入” |
| “现在 TFT 已经显示中文” | “中文字库已生成但尚未链接，当前 TFT UI 仍为英文” |
| “TFT 会显示完整的 DeepSeek 回答” | “当前 TFT 显示经过校验的枚举，完整英文建议保留在电脑日志” |
...
我们的项目叫 GreenMind，是一个基于 STM32F103 的智慧植物养护系统。设备端采集温度、空气湿度和相对光照，在 3.5 英寸触摸屏上提供主页、环境详情、AI、系统状态以及植物选择页面。ESP07 同时工作在 AP 和 STA 模式，通过电脑热点建立 TCP 通信。电脑端 Python 网关访问 SQLite 植物数据库，先根据所选植物的适宜温湿度和光照范围做确定性判断，再调用 DeepSeek 生成简短解释。模型不能改变规则结果，也不能控制水泵，STM32 只接收经过白名单校验的状态码。当前核心 AI 闭环、六种植物数据库和触摸选择已经完成，下一步是接入中文对话框、完善所有植物图片动态加载，并在增加土壤湿度后实现安全的本地水泵控制。
