...
`wifi_tft` 是此前验证成功的 WiFi 基线工程，应保持不动，作为回退和对照。综合功能只在 `AgricultureSys` 中继续开发。

API Key 位于本地 `AgricultureSys/myDeepseekApiKey.md`，热点密码位于忽略提交的 `firmware/APP/agriculture/app_secrets.h`。两者均不得复制进讨论文档、日志或公开仓库。
...
2. 不要修改 `wifi_tft` 基线工程；只修改 `AgricultureSys`。
